...
ASSETS: tuple[HeroAsset, ...] = (
    ...
    HeroAsset("/assets/img/hero-v2/robot-eoat.svg", "로봇·EOAT", ("로봇자동화", "공장자동화"), ("로봇", "그리퍼", "엔드이펙터", "eoat", "payload", "moment", "티칭"), 4),
    ...
    # Existing raster library remains as safe fallback.
...
def _asset_score(asset: HeroAsset, post: dict, used_counts: dict[str, int]) -> int:
    ...
    # Semantic relevance dominates all other considerations.
    ...
    # Specific V2 assets get a mild preference over generic raster fallback.
    if "/hero-v2/" in asset.path:
        score += 8

    # Duplication is only a penalty; it must not override a strong semantic match.
...
def select_image(post: dict, used_counts: dict[str, int] | None = None) -> tuple[str, int, str]:
    ...
    # If every semantic score is weak, prefer category/default fallback rather than a random specialist image.
    if score < 10:
