...
제목 패턴 (반드시 아래 3가지 중 하나 선택):
- [설비/기술명] + 아진네트웍스 + [성과/특징]  예) "팔레타이징 자동화, 아진네트웍스가 구현한 24시간 무인 물류"
- [문제상황] + [아진네트웍스 솔루션]          예) "라벨 오부착 불량 30% 감소 — 아진네트웍스 비전검사 솔루션"
- [키워드] 전문기업 아진네트웍스가 설명하는 [주제]  예) "AGV 전문기업 아진네트웍스가 설명하는 자율주행 물류로봇 도입 체크리스트"

반드시 JSON 형식만 반환. 마크다운 코드블록 없이 순수 JSON만:
{{
  "title": "제목 (위 3가지 패턴 중 하나)",
